...
│   ├── job-advice.md         ← 求职内参（制度核验/面试/避坑/证据框架）
│   └── model-evidence.md     ← 人员选拔与预测模型的同行评审证据
...
/workfind 中国铁塔         # 查单家单位
/workfind 攻略             # 经公开制度核实的晋升、合规推荐与人员流动信息
...
/workfind 内参 结构化面试  # 央国企求职内参（制度/合规推荐/人员流动/面试/避坑）
/workfind 投递 记录 中国铁塔 综合管理 已投  # 记录投递事件（投递 列表 查看）
/workfind 投递 结果 中国铁塔 综合管理 offer  # 记录最终结果，用于模型校准
/workfind 模型 校准        # 检查数据、样本量、验证与校准条件
/workfind 数据 贡献        # 预览最小化数据；逐次同意后才可提交
```

## 贡献数据改进模型（完全自愿、逐次确认）

数据只有在口径一致、样本量足够并完成验证后才能用于更新模型。如果你自愿贡献：

1. 跑 `/workfind 数据 贡献` → skill 生成最小化 JSON（申请月份、行业大类、岗位族、招聘环节、结果）并展示预览
2. 只有你对本次具体内容明确同意，skill 才能提交；默认仅保存在本地

> 隐私边界：不收集姓名、学校、公司、详细地点、关系与身份属性、自由文本简历；小样本组合仍可能有重新识别风险，因此优先不提交或合并。
...
- **录用证据框架**：默认输出“排除/弱/中/强”及证据；只有本地数据、样本量、验证和校准均达标时才输出概率
- **央国企求职内参**：公开制度核验、合规推荐、人员流动、结构化面试和避坑方法论
...
模型边界和方法依据来自 14 篇已核验 DOI 的同行评审论文及 SIOP 人员选拔规范，覆盖选拔效度更新、结构化面试、人岗匹配、分层模型、样本量、验证与校准。仓库不使用预印本、经验帖或跨行业漏斗作为个人概率参数。

完整 APA 7 参考文献及“可支持/不可支持”的解释见 `.reasonix/skills/workfind/references/model-evidence.md`。

...
## 投递追踪（模型实证化，2026-08 新增）

把模型从"文献拼装"升级为"有验证的实证案例"——**投递前记录预测，投递后记录结果**：

- **模板**：`docs/投递追踪模板.csv`（每投 1 岗加 1 行，投递前先填预测列，结果出来后回填结果列）
- **分析**：`node tools/calibration-analysis.js`（秋招结束后跑：校准曲线 / Brier score / 环节漏斗 / 分层诊断）
- **抽样纪律**（数据价值的生命线）：
  1. **全样本记录**，不许挑——投过的每岗都记，包括"随手投的"
  2. **前瞻性**：预测必须在投递前写死，知道结果后回填预测 = 数据作废
  3. **分层覆盖**：30 岗里硬标准/主观裁量/大厂算法各 ~10，面试结构四类都有
- **结果判定**：以官方系统状态为准（已处理/笔试通知/面试/offer），不凭感觉

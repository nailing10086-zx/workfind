...
```
/workfind 简历: 简历/我的简历.md 地区: 四川 只查已开放: true
```
...
```
/workfind 全部 四川        # 查四川全部国企（逐家含难度评估）
/workfind 中国铁塔         # 查单家单位
...
- **录用参考模型（论文基线+简历修正）**：Urology Match 等论文实证基线（申请→面试 10-12%）× 逐项简历修正（学历/证书/经历/关系，每项标论文依据）→ 个人参考区间（封顶 50%）+ 多投组合策略（5+面试→96%）

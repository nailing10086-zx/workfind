...
- **录用参考模型（论文基线+简历修正，2026-08 v2）**：通用劳动力市场简历审计（Bertrand & Mullainathan 2004 AER 等）实证基线（申请→回调 5-10%）× 笔试通过率（15-40%，央国企特有环节）× 面试→offer（按面试结构 15-50%）× 逐项简历修正（学历/证书/经历/关系，每项标论文依据，总封顶 ×2.0）→ 个人参考区间（单岗封顶 20%）+ 多投组合策略（相关性区间模型）
